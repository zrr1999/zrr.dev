...
- **版本锁定（代理改依赖前必读）**：Astro 与各 app 保持一致且当前固定为 **`6.3.1`**；Vite+ 栈通过 workspace **catalog + overrides** 固定为 **`0.1.24`**。调整 Astro 或 vite-plus 版本前，须在本地跑通 **`vp run build`**，否则可能遇到 vite-plus-core 在 `generateBundle` 中的 **`Not implemented`**（背景见 [vitejs/vite#22356](https://github.com/vitejs/vite/issues/22356)）。`@astrojs/internal-helpers` 由 override 固定在 **`0.10.0`** 以匹配 `@astrojs/mdx` v6 的 markdown helper 导出。
